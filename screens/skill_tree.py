    ...
    "production": {
        ...
        "skills": [
            ...
            },
            {
                ...
                "cost": 400,
                "prerequisites": ["building_upgrade_farm_2"],  # fixed: was mine_upgrade
...
def afficher_skill_tree(ecran, player, unlocked_skills, batiments_data):
    ...
    def build_edge_surface():
        ...
        for skill in all_skills:
            for prereq_id in skill["prerequisites"]:
                ...
                both_unlocked = (is_skill_unlocked(skill["id"], unlocked_skills) and
                                 is_skill_unlocked(prereq_id, unlocked_skills))
                if both_unlocked:
                    draw_glow_line(surf, skill["cat_color"],
                                   parent["screen_pos"], skill["screen_pos"],
                                   width=4, alpha=200)
                else:
                    pygame.draw.line(surf, (60, 60, 75),
                                     parent["screen_pos"], skill["screen_pos"], 2)
        return surf

    edge_surf = build_edge_surface()
    needs_edge_rebuild = False

    def get_state(skill):
        if is_skill_unlocked(skill["id"], unlocked_skills):
            return "unlocked"
        prereqs_met = all(is_skill_unlocked(p, unlocked_skills) for p in skill["prerequisites"])
    ...
    while en_menu:
        ...
        for skill in all_skills:
            ...
            if state == "unlocked":
                draw_glow_circle(ecran, skill["cat_color"], (cx, cy), r, alpha=240)
            elif state == "locked":
                draw_glow_circle(ecran, skill["cat_color"], (cx, cy), r, alpha=90)
            else:
        ...
        for cat_id, cat_data in SKILLS_DATA.items():
            positions = [s["screen_pos"] for s in all_skills if s["cat_id"] == cat_id]
            if not positions:
                continue
            avg_x = sum(p[0] for p in positions) // len(positions)
            max_y = max(p[1] for p in positions) + 36
            label = font_cat.render(cat_data["name"].upper(), True, cat_data["color"])
            ecran.blit(label, (avg_x - label.get_width() // 2, min(max_y, H - 30)))
